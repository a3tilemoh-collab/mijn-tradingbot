# ============================================================================
#  ELITE TRADING BOT v2.4 - AGGRESSIVE LONG-ONLY TEST ENGINE
#  Markt: Crypto (Volledig) + Metalen + Forex | Account: Pepperstone
#  Strategie: Agressieve M15 Buy Trigger (Shorts Uitgeschakeld)
# ============================================================================
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('elite_long_bot.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)
log = logging.getLogger(__name__)

# ─── Config (AGRESSIEF + RUIME SPREADS) ──────────────────────────────────────
@dataclass
class Config:
    BALANCE_RISK_MIN:   float = 0.03   # Direct 3% risico per trade
    BALANCE_RISK_MAX:   float = 0.06   
    MAX_DAILY_LOSS_PCT: float = 0.15   # 15% ademruimte voor testfase
    MAX_OPEN_TRADES:    int   = 6      
    MAX_TRADES_PER_DAY: int   = 50     
    CONSEC_LOSS_LIMIT:  int   = 8      

    TRAIL_TRIGGER_R:    float = 0.8    # Snel winst beveiligen
    TRAIL_STEP_R:       float = 0.4    
    PARTIAL_CLOSE_R:    float = 1.2    

    MAX_SPREAD:         dict  = field(default_factory=lambda: {
        'crypto': 9999, 'metals': 9999, 'forex': 9999  # Spread filter staat UIT
    })

    TF_TREND:  int = mt5.TIMEFRAME_H4
    TF_SETUP:  int = mt5.TIMEFRAME_H1
    TF_ENTRY:  int = mt5.TIMEFRAME_M15
    MAGIC:     int = 999111  # Nieuw Magic Number voor deze Long-Only run
...
def connect():
    ...
    log.info(f"🚀 AGGRESSIVE LONG BOT LIVE | Account: {acc.login} | Balance: €{acc.balance:,.2f}")
...
def analyze(symbol) -> Optional[Signal]:
    ...
    # --- INGEKORTE CONFLUENCY: Alleen berekenen voor LONG ---
    ...
    if candle_size > LM['atr'] * 0.4: 
        if LM['close'] > LM['open']: score_buy += 1

    atr = LM['atr']

    # Trigger vanaf 4 punten confluency
    if score_buy >= 4:
...
def open_trade(signal: Signal) -> bool:
    ...
    req = {
        ...
        "magic":        CFG.MAGIC,
        "comment":      f"AgressiveLong|{signal.score}",
    ...
    if result.retcode == mt5.TRADE_RETCODE_DONE:
        log.info(f"🔥 LONG ORDER EXECUTED {sym} | lot={lot} | score={signal.score}/10")
        ...
        if profit_r >= CFG.PARTIAL_CLOSE_R and ticket not in state['partial_closed']:
            ...
            if close_volume < pos.volume:
                req = {
                    ...
                    "type":     mt5.ORDER_TYPE_SELL, "position": ticket, "price": t.bid,
                    "magic":    CFG.MAGIC, "comment": "Long|Partial",
...
def check_risk() -> bool:
    ...
    if state['daily_trades'] >= CFG.MAX_TRADES_PER_DAY: return False
...
def track_closed_trades():
    ...
    if last.ticket not in state['verwerkte_deals']:
        ...
        if last.profit < 0: state['consec_losses'] += 1
...
def main():
    ...
    while True:
        try:
            ...
            if check_risk():
                log.info(f"🔍 [AGRESSIVE LONG SCAN] #{cycle} | Open: {len(positions())}/{CFG.MAX_OPEN_TRADES}")
                for symbol in ALL_PAIRS:
                    ...
                    if signal:
                        open_trade(signal)
                        time.sleep(1)
            time.sleep(10)  # Snelle 10-seconden loop
